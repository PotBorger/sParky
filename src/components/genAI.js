...
async function loadCoordinates() {
    ...
    const coordsPath = path.join(scriptDir, '..','components', 'currentCoord.json');
    ...
    try {
        ...
        if (!latitude || !longitude) {
            throw new Error("Both 'currentLat' and 'currentLon' must be present in currentCoords.js");
        }
        ...
        if (error.code === 'ENOENT') {
            ...
            throw new Error(`Error loading coordinates from ${coordsPath}: ${error.message}`);
        }
    }
}
...
async function fetchCurrentAQI(latitude, longitude) {
    ...
    try {
        ...
        if (aqiValue === undefined || aqiValue === null) {
            throw new Error(`'currentAQI' field missing in backend response: ${JSON.stringify(data)}`);
        }
        
        return aqiValue;
    } catch (error) {
        if (error.response) {
            throw new Error(`Failed to fetch AQI from backend: HTTP ${error.response.status} - ${error.response.statusText}`);
...
// -------------------------------------------------------------------
function buildPrompt(aqiValue) {
    return `My current AQI is ${aqiValue} and there is a wildfire 100 km away from me. ` +
...
async function saveResponse(aiJson) {
    const scriptDir = __dirname;
    const outputPath = path.resolve(path.join(scriptDir, '..', 'components', 'response.json'));
...
// -------------------------------------------------------------------
export async function generateWildfireImpact() {
    try {
        console.log('Starting wildfire impact analysis...');
        
        // Load coordinates
        console.log('📍 Loading coordinates...');
        const { latitude, longitude } = await loadCoordinates();
        ...
        const prompt = buildPrompt(aqiValue);
        
        // Invoke AI model
        const aiJson = await invokeBedrockModel(prompt);
